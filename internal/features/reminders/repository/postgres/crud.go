...
func (r *RemindersRepositoryImpl) CreateReminders(ctx context.Context, reminders []*domain.Reminder) error {
	...
	const sql = `
		INSERT INTO reminders (id, user_id, entity_type, entity_id, remind_at, is_sent, created_at)
	...
	for i, rem := range reminders {
		...
		rem.IsSent = false
		batch[i] = []any{rem.ID, rem.UserID, rem.EntityType, rem.EntityID, rem.RemindAt, rem.IsSent, rem.CreatedAt}
	}
...
func (r *RemindersRepositoryImpl) GetPending(ctx context.Context, before time.Time, limit int) ([]*domain.Reminder, error) {
	const sql = `
		SELECT id, user_id, entity_type, entity_id, remind_at, is_sent, sent_at, created_at
		FROM reminders
