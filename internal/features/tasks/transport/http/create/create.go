...
func (h *Handler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	...
	task, err := h.svc.CreateTask(ctx, userID, input)
	if err != nil {
		core_response.NewHTTPResponseHandler(nil, w).ErrorResponse(err, "failed to create task")
		return
	}

	respHandler := core_response.NewHTTPResponseHandler(nil, w)
	respHandler.JSONResponse(task, http.StatusCreated)
}
